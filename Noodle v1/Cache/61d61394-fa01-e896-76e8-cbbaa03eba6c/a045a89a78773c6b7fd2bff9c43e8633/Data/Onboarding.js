...
//@input SceneObject OnboardingContainer {"label": "Onboarding Container"}
...
function nextImage() {
    // Only fade out if not the last image
    var isLastImage = (currentIndex === script.imageComponents.length - 1) && !script.loopSequence;

    if (!isLastImage && script.imageComponents[currentIndex]) {
    ...
    if (currentIndex >= script.imageComponents.length) {
        if (script.loopSequence) {
            ...
            isComplete = true;
            isPlaying = false;
            // Keep last image visible at full opacity
            if (script.imageComponents[script.imageComponents.length - 1]) {
                setImageAlpha(script.imageComponents[script.imageComponents.length - 1], 1.0);
            }
        }
    } else {
        showImage(currentIndex);
...
function calculateAlpha(t, isLastImage) {
    ...
    if (t < fadeIn) {
        ...
        // Fade out (skip for last image)
        if (isLastImage) {
            return 1.0; // Keep at full opacity
        }
