...
// TODO: Will be added step by step
...
export class NodeManager extends BaseScriptComponent {
    @input
    @hint("NodeMenu component for showing node creation options (optional)")
    public nodeMenu: NodeMenu | null = null;
    ...
    onStart() {
        ...
        if (this.nodeMenu) {
            ...
            if (this.nodeMenu.onNodeTypeSelected && typeof this.nodeMenu.onNodeTypeSelected.add === 'function') {
                ...
                print("NodeManager: Warning - NodeMenu.onNodeTypeSelected Event not properly initialized.");
            }
        } else {
    ...
    createNode(nodeType: string, position: vec3): SceneObject | null {
        ...
        if (nodeObject) {
            ...
            const baseNode = nodeObject.getComponent(BaseNode.getTypeName() as any) as BaseNode;
            if (baseNode) {
    ...
    private onNodeTypeSelectedFromMenu(nodeType: string): void {
        ...
        if (newNode) {
            const baseNode = newNode.getComponent(BaseNode.getTypeName() as any) as BaseNode;
            if (baseNode) {
    ...
    serialize(): any {
        ...
        this.nodes.forEach((node, nodeId) => {
            const baseNode = node.getComponent(BaseNode.getTypeName() as any) as BaseNode;
            if (baseNode) {
        ...
        if (data.connections) {
            data.connections.forEach((connectionData: any) => {
                ...
                if (sourceNode && targetNode) {
                    const sourceBaseNode = sourceNode.getComponent(BaseNode.getTypeName() as any) as BaseNode;
                    const targetBaseNode = targetNode.getComponent(BaseNode.getTypeName() as any) as BaseNode;
