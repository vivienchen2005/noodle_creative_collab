...
let BaseNode = (() => {
    ...
    var BaseNode = _classThis = class extends _classSuper {
        ...
        onAwake() {
            ...
            // Auto-register with NodeManager if it exists
            this.registerWithNodeManager();
         ...
         */
        isNearOutPoint(worldPosition, threshold = 0.05) {
         ...
         */
        isNearInPoint(worldPosition, threshold = 0.05) {
            const inPos = this.getInConnectionPosition();
            const distance = inPos.sub(worldPosition).length;
            return distance < threshold;
        }
        /**
         * Registers this node with the NodeManager (if it exists)
         */
        registerWithNodeManager() {
            // Try to find NodeManager and register this node
            // Use dynamic require to avoid circular dependency
            try {
                const NodeManagerModule = require("./NodeManager");
                const manager = NodeManagerModule.NodeManager.getInstance();
                if (manager) {
                    manager.registerNode(this.sceneObject, this);
                }
            }
            catch (e) {
                // NodeManager might not be available yet, that's okay
                // It will be registered when NodeManager initializes
            }
        }
    };
