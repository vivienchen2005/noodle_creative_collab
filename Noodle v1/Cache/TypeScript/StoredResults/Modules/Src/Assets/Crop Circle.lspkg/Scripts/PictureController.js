...
const CropRegion_1 = require("./CropRegion");
let PictureController = (() => {
    ...
    var PictureController = _classThis = class extends _classSuper {
         ...
         */
        assignCameraServiceToCropRegion(scanner) {
            ...
            if (!cameraService) {
                ...
                for (let i = 0; i < rootObjects; i++) {
                    ...
                    const service = rootObject.getComponent(CameraService_1.CameraService.getTypeName());
                    if (service) {
                    ...
                    for (let j = 0; j < childCount; j++) {
                        ...
                        const childService = child.getComponent(CameraService_1.CameraService.getTypeName());
                        if (childService) {
         ...
         */
        findCropRegionInHierarchy(obj) {
            // Check current object
            const cropRegion = obj.getComponent(CropRegion_1.CropRegion.getTypeName());
            if (cropRegion) {
                return cropRegion;
            }
