...
export class NodeConnectionController extends BaseScriptComponent {
    ...
    private _pendingSourceButtonId: string | null = null;
     ...
     */
    public onInputNodeButtonClicked(sourceNode: SceneObject, sourceType: "text" | "image", buttonId: string = ""): void {
     ...
     */
    public onProcessNodeInputClicked(targetNode: SceneObject, inputType: "text" | "image", targetButtonId: string = ""): boolean {
        ...
        print(`NodeConnectionController: Attempting to connect ${this._pendingSourceType} from ${this._pendingSourceNode.name} (Button: ${this._pendingSourceButtonId}) to ${targetNode.name} (Button: ${targetButtonId})`);
        ...
        if (imageGenNode) {
            if (inputType === "text") {
                connected = imageGenNode.connectTextInput(this._pendingSourceNode, this._pendingSourceButtonId || "");
            } else if (inputType === "image") {
                connected = imageGenNode.connectImageInput(this._pendingSourceNode, this._pendingSourceButtonId || "");
            }
        }
        ...
        if (!connected) {
            ...
            if (process3DNode) {
                if (inputType === "text") {
                    connected = process3DNode.connectTextInput(this._pendingSourceNode, this._pendingSourceButtonId || "");
                } else if (inputType === "image") {
                    connected = process3DNode.connectImageInput(this._pendingSourceNode, this._pendingSourceButtonId || "");
                }
            }
        }
        ...
        if (connected) {
            ...
            print(`NodeConnectionController: Connection failed - target node not found or connection already exists`);
            this.clearPendingConnection();
            return false;
        }
    }
