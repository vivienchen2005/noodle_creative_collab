...
export class InputNodeImage extends BaseScriptComponent {
    ...
    private cloneAndStoreMaterial(): void {
        if (!this.imageComponent) {
            print("InputNodeImage: Cannot clone material, imageComponent is null (will retry when material is assigned)");
            return;
        }
        ...
        try {
            ...
            if (!mainPass) {
                print("InputNodeImage: Image component has no mainPass (will retry when material is assigned)");
                return;
            }
            ...
            if (!originalMaterial) {
                print("InputNodeImage: Image component has no material yet (this is OK - material will be cloned when assigned)");
                return;
            }
    ...
    private findOrCreateConnectionController(): void {
        ...
        if (controller) {
            print(`InputNodeImage: NodeConnectionController created`);
            controller.onInputNodeButtonClicked(this.sceneObject, "image", this._outputButtonId);
        }
    }
