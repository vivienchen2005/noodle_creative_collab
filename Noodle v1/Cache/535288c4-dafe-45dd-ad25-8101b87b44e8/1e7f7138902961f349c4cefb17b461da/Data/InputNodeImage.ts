            ...
            try {
                ...
                if (captureRendMesh && captureRendMesh.mainPass && captureRendMesh.mainPass.captureImage) {
                    ...
                    try {
                        ...
                        // It should be a static snapshot, but we need to use it correctly
                        const captureImage = captureRendMesh.mainPass.captureImage;
                        
                        // Try to use captureImage directly as a texture
                        // ProceduralTextureProvider can sometimes be used as Texture in Lens Studio
                        // But we need to make sure it's actually static
                        staticTexture = captureImage as any as Texture;
                        print(`InputNodeImage: Using captureImage directly as texture (should be static snapshot)`);
                        
                        // If that doesn't work visually, try to get the static frame from PictureBehavior
                        if ((pictureBehavior as any).getCapturedStaticTexture && typeof (pictureBehavior as any).getCapturedStaticTexture === 'function') {
                            ...
                            if (staticFrame) {
                                // We have the static camera frame, but it's not cropped
                                // For now, use it (better than live feed)
                                print(`InputNodeImage: Also have static camera frame from PictureBehavior (full frame, not cropped)`);
                                // Keep using captureImage as it should have the crop applied
                            }
                        }
                    } catch (extractError) {
