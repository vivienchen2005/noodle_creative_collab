...
export class InputNodeImage extends BaseScriptComponent {
    ...
    @input
    @hint("Crop Circle prefab (Scanner.prefab from Crop Circle.lspkg) - each InputNodeImage gets its own instance")
    @allowUndefined
    public cropCirclePrefab: ObjectPrefab | null = null;
    ...
    private _currentCropCircleInstance: SceneObject | null = null; // Each InputNodeImage has its own crop circle instance
    ...
    private onCaptureButtonClicked(): void {
        print(`InputNodeImage: Capture button clicked - creating crop circle instance for ${this.sceneObject.name}`);

        // Destroy any existing crop circle instance first
        if (this._currentCropCircleInstance) {
            this._currentCropCircleInstance.destroy();
            this._currentCropCircleInstance = null;
        }

        // Check if we have a crop circle prefab
        if (!this.cropCirclePrefab) {
            print("InputNodeImage: ERROR - Crop Circle prefab not assigned! Please assign Scanner.prefab in the inspector.");
            return;
        }

        // Create a new crop circle instance for this InputNodeImage
        // Parent it to scene root so it's visible and independent
        const rootObject = global.scene.getRootObject(0);
        this._currentCropCircleInstance = this.cropCirclePrefab.instantiate(rootObject);
        this._currentCropCircleInstance.enabled = true;
        this._currentCropCircleInstance.name = `CropCircle_${this.sceneObject.name}_${Date.now()}`;

        print(`InputNodeImage: Crop circle instance created and enabled for ${this.sceneObject.name}`);
    ...
    private setupCaptureListener(): void {
        ...
        // Find PictureBehavior component in the crop circle instance
        const pictureBehavior = this.findPictureBehaviorInHierarchy(this._currentCropCircleInstance);
        if (!pictureBehavior) {
            print(`InputNodeImage: WARNING - PictureBehavior not found in crop circle instance for ${this.sceneObject.name}`);
            return;
        }
        ...
        this.createEvent("UpdateEvent").bind(() => {
            ...
            if (!this._currentCropCircleInstance || isNull(this._currentCropCircleInstance)) {
                callbackExecuted = true;
                return; // Crop circle instance was destroyed
            }

            checkCount++;
            if (checkCount > maxChecks) {
                print(`InputNodeImage: Capture timeout for ${this.sceneObject.name} - destroying crop circle instance`);
                if (this._currentCropCircleInstance) {
                    this._currentCropCircleInstance.destroy();
                    this._currentCropCircleInstance = null;
                }
            ...
            try {
                ...
                if (captureRendMesh && captureRendMesh.mainPass && captureRendMesh.mainPass.captureImage) {
                    ...
                    if (screenCropTexture) {
                        print(`InputNodeImage: Capture detected for ${this.sceneObject.name}! Getting cropped texture from screenCropTexture...`);
                        callbackExecuted = true;
                        this.onCaptureComplete(screenCropTexture);
                        // Destroy crop circle instance after capture
                        if (this._currentCropCircleInstance) {
                            this._currentCropCircleInstance.destroy();
                            this._currentCropCircleInstance = null;
                            print(`InputNodeImage: Crop circle instance destroyed after capture for ${this.sceneObject.name}`);
                        }
                    }
                }
            } catch (e) {
    ...
    private onCaptureComplete(croppedTexture: Texture): void {
        ...
        try {
            if (!croppedTexture) {
                print("InputNodeImage: WARNING - Cropped texture is null");
                return;
            }
            ...
            // Clone material FIRST (before setting texture) to ensure independence
            // This ensures each InputNodeImage has its own material copy
            this.cloneAndStoreMaterial();

            // Wait a frame to ensure material is cloned, then set texture
            let frameCount = 0;
            let callbackExecuted = false;
            this.createEvent("UpdateEvent").bind(() => {
                if (callbackExecuted) return; // Only execute once
                ...
                // Wait at least 1 frame to ensure material cloning is complete
                if (frameCount < 1) {
                    return;
                }
                ...
                // Ensure material is cloned (retry if needed)
