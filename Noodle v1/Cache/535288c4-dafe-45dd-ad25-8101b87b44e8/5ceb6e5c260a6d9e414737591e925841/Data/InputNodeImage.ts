    ...
    private setupCaptureListener(): void {
        ...
        this.createEvent("UpdateEvent").bind(() => {
            ...
            try {
                ...
                const picAnchorObj = (pictureBehavior as any).picAnchorObj;
                const loadingObj = (pictureBehavior as any).loadingObj;
                ...
                if (captureRendMesh && captureRendMesh.mainPass && captureRendMesh.mainPass.captureImage) {
                    ...
                    if (screenCropTexture) {
                        ...
                        // Hide ALL visual elements that display the captured image on the crop circle
                        // We only want the texture passed to InputNodeImage, NOT displayed on crop circle
                        if (picAnchorObj) {
                            picAnchorObj.enabled = false;
                            print(`InputNodeImage: Hidden picAnchorObj (captured image visual) for ${this.sceneObject.name}`);
                        }
                        if (captureRendMesh && captureRendMesh.sceneObject) {
                            captureRendMesh.sceneObject.enabled = false;
                            print(`InputNodeImage: Hidden captureRendMesh (captured image visual) for ${this.sceneObject.name}`);
                        }
                        if (loadingObj) {
                            loadingObj.enabled = false;
                            print(`InputNodeImage: Hidden loadingObj for ${this.sceneObject.name}`);
                        }

                        callbackExecuted = true;
                        this._isCapturing = false;

                        // Pass the cropped texture to InputNodeImage (not displayed on crop circle)
