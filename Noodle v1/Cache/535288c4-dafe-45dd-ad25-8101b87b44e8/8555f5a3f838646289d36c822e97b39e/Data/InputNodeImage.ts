...
export class InputNodeImage extends BaseScriptComponent {
    ...
    private setupCapturePolling(pictureBehavior: PictureBehavior): void {
        ...
        this.createEvent("UpdateEvent").bind(() => {
            ...
            // When captureImage is set, we need to get a static snapshot of the cropped texture
            // screenCropTexture is a live feed, so we need to copy the frame to get a static image
            try {
                ...
                if (captureRendMesh && captureRendMesh.mainPass && captureRendMesh.mainPass.captureImage) {
                    // Capture is complete - we need to get a static copy of the cropped texture
                    // screenCropTexture is a live feed, so we need to copy the frame
                    if (screenCropTexture) {
                        print(`InputNodeImage: Capture detected for ${this.sceneObject.name}! Getting static snapshot of cropped texture...`);

                        // Try to copy the frame from screenCropTexture to get a static snapshot
                        let staticTexture: Texture | null = null;
                        try {
                            // Check if screenCropTexture has copyFrame method (for CameraTexture)
                            if ((screenCropTexture as any).copyFrame && typeof (screenCropTexture as any).copyFrame === 'function') {
                                staticTexture = (screenCropTexture as any).copyFrame();
                                print(`InputNodeImage: Copied frame from screenCropTexture`);
                            } else {
                                // If copyFrame doesn't exist, try to get the texture from the control
                                // The screenCropTexture control should have the cropped result
                                const cropControl = screenCropTexture.control as any;
                                if (cropControl && cropControl.inputTexture) {
                                    // Try to copy from the input texture
                                    const inputTexture = cropControl.inputTexture;
                                    if (inputTexture && (inputTexture as any).copyFrame && typeof (inputTexture as any).copyFrame === 'function') {
                                        const copiedInput = (inputTexture as any).copyFrame();
                                        // Create a new texture with the crop applied
                                        // For now, use the screenCropTexture directly but we'll need to make it static
                                        print(`InputNodeImage: WARNING - Cannot get static copy, using screenCropTexture (may show live feed)`);
                                        staticTexture = screenCropTexture; // Fallback - will need better solution
                                    } else {
                                        print(`InputNodeImage: WARNING - Cannot copy frame, using screenCropTexture directly`);
                                        staticTexture = screenCropTexture; // Fallback
                                    }
                                } else {
                                    // Last resort: use screenCropTexture but it will be live
                                    print(`InputNodeImage: WARNING - Using screenCropTexture directly (will show live feed - need to fix)`);
                                    staticTexture = screenCropTexture;
                                }
                            }
                        } catch (copyError) {
                            print(`InputNodeImage: Error copying frame: ${copyError}, using screenCropTexture directly`);
                            staticTexture = screenCropTexture; // Fallback
                        }
                        ...
                        // screenCropTexture doesn't exist
                        if (checkCount % 60 == 0) { // Log every second
                            print(`InputNodeImage: Waiting for screenCropTexture...`);
                        }
                    }
                }
            } catch (e) {
