...
export class InputNodeImage extends BaseScriptComponent {
     ...
     */
    private cleanupOldScanners(): void {
        if (!this.cropCircleObject) return;
        ...
        if (!pictureController) {
            print(`InputNodeImage: PictureController not found, skipping cleanup`);
            return;
        }

        // Scanners are instantiated as children of PictureController
        const childCount = pictureController.sceneObject.getChildrenCount();
        const objectsToDestroy: SceneObject[] = [];

        for (let i = 0; i < childCount; i++) {
            const child = pictureController.sceneObject.getChild(i);
            
            // Skip objects that look like permanent parts of the prefab (not instantiated scanners)
            // Scanners are dynamically created and usually have names like "PictureController" or prefab name
            const childName = child.name.toLowerCase();
            if (childName.includes("controller") || childName.includes("service") || childName.includes("camera")) {
                continue; // Don't destroy controllers/services
            }
            
            // Check if this child has a completed PictureBehavior
     ...
     */
    private findPictureControllerInHierarchy(obj: SceneObject): any | null {
        if (!obj || isNull(obj)) {
            return null;
        }
        ...
        const components = obj.getAllComponents("Component.ScriptComponent");
        for (const comp of components) {
            const script = comp as ScriptComponent;
            // Check if this is a PictureController by looking for its methods/properties
            if (script && (script as any).createScanner && (script as any).scannerPrefab) {
                return script;
            }
        }
     ...
     */
    private findActivePictureBehavior(obj: SceneObject): PictureBehavior | null {
        if (!obj || isNull(obj)) {
            return null;
        }
            ...
            try {
                ...
                if (pictureBehavior.isCaptureComplete()) {
                    ...
                    if (!capturedTexture) {
                        ...
                        if (captureImage) {
                            capturedTexture = captureImage as any as Texture;
     ...
     */
    private updateCaptureButtonLabel(label: string): void {
