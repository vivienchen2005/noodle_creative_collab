        ...
        this.createEvent("UpdateEvent").bind(() => {
            ...
            try {
                ...
                if (captureRendMesh && captureRendMesh.mainPass && captureRendMesh.mainPass.captureImage) {
                    ...
                    try {
                        // Try to get texture from ProceduralTextureProvider
                        // ProceduralTextureProvider can sometimes be used directly, but we need the actual texture
                        if ((captureImage as any).texture) {
                            ...
                            // If we can't extract texture, try to get it from the underlying screenCropTexture
                            // But we need to copy the frame to make it static
                            if (screenCropTexture) {
                                // Try to get the input texture from screenCropTexture and copy it
                                const cropControl = screenCropTexture.control as any;
                                if (cropControl && cropControl.inputTexture) {
                                    const inputTexture = cropControl.inputTexture;
                                    // Check if inputTexture has copyFrame (CameraTexture)
                                    if ((inputTexture as any).copyFrame && typeof (inputTexture as any).copyFrame === 'function') {
                                        // Copy the frame to get a static snapshot
                                        const copiedFrame = (inputTexture as any).copyFrame();
                                        // Now apply the crop to this copied frame
                                        // For now, use the copied frame directly (it's the full camera frame, not cropped)
                                        // TODO: Apply crop region to copied frame
                                        staticTexture = copiedFrame;
                                        print(`InputNodeImage: Copied frame from input texture (full frame, crop may not be applied)`);
                                    } else {
                                        // Can't copy, use screenCropTexture as fallback (will be live)
                                        print(`InputNodeImage: WARNING - Cannot copy frame, using screenCropTexture (will show live feed)`);
                                        staticTexture = screenCropTexture;
                                    }
                                } else {
                                    // Last resort: use screenCropTexture (will be live)
                                    print(`InputNodeImage: WARNING - Using screenCropTexture directly (will show live feed)`);
                                    staticTexture = screenCropTexture;
                                }
                            }
                        }
                        ...
                        // Fallback to screenCropTexture
                        if (screenCropTexture) {
                            staticTexture = screenCropTexture;
                            print(`InputNodeImage: Using screenCropTexture as fallback (will show live feed)`);
                        }
                    }
