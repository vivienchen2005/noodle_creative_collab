        ...
        this.createEvent("UpdateEvent").bind(() => {
            ...
            try {
                ...
                if (captureRendMesh && captureRendMesh.mainPass && captureRendMesh.mainPass.captureImage) {
                    ...
                    try {
                        // Try to get the static captured texture from PictureBehavior
                        if ((pictureBehavior as any).getCapturedStaticTexture && typeof (pictureBehavior as any).getCapturedStaticTexture === 'function') {
                            ...
                            if (staticTexture) {
                                print(`InputNodeImage: Got static texture from PictureBehavior.getCapturedStaticTexture()`);
                            } else {
                                print(`InputNodeImage: WARNING - getCapturedStaticTexture returned null`);
                            }
                        }
                        
                        // If that didn't work, try to copy frame from camera texture
                        if (!staticTexture && screenCropTexture) {
                            ...
                            if (cropControl && cropControl.inputTexture) {
                                ...
                                if ((camTexture as any).copyFrame && typeof (camTexture as any).copyFrame === 'function') {
                                    ...
                                    print(`InputNodeImage: Fallback - Copied static frame from camera texture (full frame, not cropped)`);
                                }
                            }
                        }
                        ...
                        if (screenCropTexture) {
                            staticTexture = screenCropTexture;
                            print(`InputNodeImage: Using screenCropTexture as error fallback (will show live feed)`);
                        }
                    }
