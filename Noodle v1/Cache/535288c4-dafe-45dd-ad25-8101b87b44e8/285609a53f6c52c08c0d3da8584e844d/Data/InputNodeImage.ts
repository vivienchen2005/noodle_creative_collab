        ...
        this.createEvent("UpdateEvent").bind(() => {
            ...
            try {
                ...
                if (captureRendMesh && captureRendMesh.mainPass && captureRendMesh.mainPass.captureImage) {
                    // Capture is complete - captureImage is a ProceduralTextureProvider with the static snapshot
                    // We need to get the underlying texture from it
                    print(`InputNodeImage: Capture detected for ${this.sceneObject.name}! Getting static texture from captureImage...`);

                    let staticTexture: Texture | null = null;
                    const captureImage = captureRendMesh.mainPass.captureImage;
                    
                    try {
                        // captureImage is a ProceduralTextureProvider created from screenCropTexture at capture time
                        // We need to get the underlying static texture from it
                        // First, try to get the texture from the ProceduralTextureProvider
                        if ((captureImage as any).texture) {
                            staticTexture = (captureImage as any).texture;
                            print(`InputNodeImage: Got texture from captureImage.texture`);
                        } else if ((captureImage as any).getTexture && typeof (captureImage as any).getTexture === 'function') {
                            staticTexture = (captureImage as any).getTexture();
                            print(`InputNodeImage: Got texture from captureImage.getTexture()`);
                        } else {
                            // ProceduralTextureProvider might have the source texture
                            // Try to access the source texture that was used to create it
                            if ((captureImage as any).sourceTexture) {
                                const sourceTexture = (captureImage as any).sourceTexture;
                                // If source is screenCropTexture (live), we need to copy the frame from the camera
                                // Get the camera texture from CameraService
                                const cropControl = screenCropTexture.control as any;
                                if (cropControl && cropControl.inputTexture) {
                                    ...
                                    if ((camTexture as any).copyFrame && typeof (camTexture as any).copyFrame === 'function') {
                                        staticTexture = (camTexture as any).copyFrame();
                                        print(`InputNodeImage: Copied static frame from camera texture`);
                                    } else {
                                        print(`InputNodeImage: WARNING - Camera texture has no copyFrame, using screenCropTexture (will be live)`);
                                        staticTexture = screenCropTexture;
                                    }
                                } else {
                                    print(`InputNodeImage: WARNING - Cannot access camera texture, using screenCropTexture (will be live)`);
                                    staticTexture = screenCropTexture;
                                }
                            } else {
                                // Last resort: use captureImage directly as texture (might work)
                                print(`InputNodeImage: Trying to use captureImage directly as texture...`);
                                staticTexture = captureImage as any as Texture;
                            }
                        }
                    } catch (extractError) {
                        print(`InputNodeImage: Error extracting texture from captureImage: ${extractError}`);
                        // Fallback: try to copy frame from camera texture
                        try {
                            const cropControl = screenCropTexture.control as any;
                            if (cropControl && cropControl.inputTexture) {
                                const camTexture = cropControl.inputTexture;
                                if ((camTexture as any).copyFrame && typeof (camTexture as any).copyFrame === 'function') {
                                    staticTexture = (camTexture as any).copyFrame();
                                    print(`InputNodeImage: Fallback - Copied static frame from camera texture`);
                                }
                            }
                        } catch (fallbackError) {
                        ...
                        if (!staticTexture && screenCropTexture) {
                            staticTexture = screenCropTexture;
                            print(`InputNodeImage: Using screenCropTexture as final fallback (WARNING: will show live feed)`);
                        }
                    }
