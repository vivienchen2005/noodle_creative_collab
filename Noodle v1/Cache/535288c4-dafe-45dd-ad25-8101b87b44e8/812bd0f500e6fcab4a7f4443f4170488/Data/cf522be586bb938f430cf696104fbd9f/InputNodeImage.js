...
let InputNodeImage = (() => {
    ...
    var InputNodeImage = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.pictureController = this.pictureController;
        ...
        setupCaptureButton() {
            ...
            this.captureButton = this._captureButtonObject.createComponent(RoundButton_1.RoundButton.getTypeName());
            if (this.captureButton) {
            ...
            this.createEvent("UpdateEvent").bind(() => {
                ...
                try {
                    ...
                    if (captureRendMesh && captureRendMesh.mainPass && captureRendMesh.mainPass.captureImage) {
                        const capturedTexture = captureRendMesh.mainPass.captureImage;
                        if (capturedTexture) {
         ...
         */
        onCaptureComplete(capturedTextureProvider) {
            print("InputNodeImage: Processing captured image...");
            // Get the texture from ProceduralTextureProvider
            // ProceduralTextureProvider can be used directly as a texture source
            try {
                // Access the underlying texture through the provider
                // In Lens Studio, ProceduralTextureProvider can be used directly
                // We need to get the actual Texture from it
                let texture = null;
                // Try to access the texture property if it exists
                if (capturedTextureProvider.texture) {
                    texture = capturedTextureProvider.texture;
                }
                else if (capturedTextureProvider.getTexture) {
                    texture = capturedTextureProvider.getTexture();
                }
                else {
                    // If we can't get texture directly, try to use the provider itself
                    // Some systems allow using ProceduralTextureProvider as Texture
                    print("InputNodeImage: Attempting to use ProceduralTextureProvider directly...");
                    // We'll need to get the texture from the RenderMeshVisual's mainPass
                    // Let's get it from the PictureBehavior's captureRendMesh instead
                    const pictureBehavior = this.findPictureBehaviorInHierarchy(this._currentScanner);
                    if (pictureBehavior) {
                        const captureRendMesh = pictureBehavior.captureRendMesh;
                        if (captureRendMesh && captureRendMesh.mainPass) {
                            // The captureImage is the ProceduralTextureProvider
                            // We need to get the actual texture from the render mesh
                            const baseTex = captureRendMesh.mainPass.baseTex;
                            if (baseTex) {
                                texture = baseTex;
                            }
                        }
                ...
                if (texture) {
                    ...
                    print("InputNodeImage: WARNING - Could not extract texture from ProceduralTextureProvider");
                }
            }
            catch (e) {
