...
export class InputNodeImage extends BaseScriptComponent {
     ...
     */
    private setupCapturePolling(pictureBehavior: PictureBehavior): void {
        if (!this._isCapturing) {
            return;
        }
            ...
            try {
                ...
                if (captureRendMesh && captureRendMesh.mainPass && captureRendMesh.mainPass.captureImage) {
                    ...
                    try {
                        ...
                        // The issue: ProceduralTextureProvider might still reference the live texture
                        // Solution: Use it directly, but also try to get the actual rendered texture
                        const captureImage = captureRendMesh.mainPass.captureImage;

                        // Try multiple approaches to get a static texture:
                        // 1. Use captureImage directly (might work if it's truly static)
                        staticTexture = captureImage as any as Texture;
                        print(`InputNodeImage: Using captureImage directly as texture`);

                        // 2. Try to get the source texture from the ProceduralTextureProvider
                        // If it has a sourceTexture property, we might be able to use that
                        if ((captureImage as any).sourceTexture) {
                            const sourceTex = (captureImage as any).sourceTexture;
                            // If source is the static frame we set, use that
                            const staticFrame = (pictureBehavior as any).getCapturedStaticTexture();
                            if (staticFrame && sourceTex === staticFrame) {
                                // Source is the static frame, but it's not cropped
                                // We need the cropped version, so keep using captureImage
                                print(`InputNodeImage: Source is static frame (not cropped), using captureImage`);
                            }
                        }

                        // 3. Try to get texture from the hidden Image component if it exists
                        // The hidden image should have rendered the cropped texture
                        if (!staticTexture || (staticTexture as any).copyFrame) {
                            // If captureImage is still live, try to get from hidden image
                            const hiddenImage = (pictureBehavior as any)._hiddenCaptureImage;
                            if (hiddenImage && hiddenImage.mainPass && hiddenImage.mainPass.baseTex) {
                                const hiddenTex = hiddenImage.mainPass.baseTex;
                                // Try to copy frame from hidden texture if possible
                                if ((hiddenTex as any).copyFrame && typeof (hiddenTex as any).copyFrame === 'function') {
                                    staticTexture = (hiddenTex as any).copyFrame();
                                    print(`InputNodeImage: Copied static frame from hidden image`);
                                } else {
                                    // Use hidden texture directly (might still be live)
                                    staticTexture = hiddenTex;
                                    print(`InputNodeImage: Using hidden image texture (may still be live)`);
                                }
                            }
                        }
                    } catch (extractError) {
                        print(`InputNodeImage: Error getting static texture: ${extractError}`);
                        // Fallback: try to get static frame from PictureBehavior
                        try {
                            if ((pictureBehavior as any).getCapturedStaticTexture && typeof (pictureBehavior as any).getCapturedStaticTexture === 'function') {
                                staticTexture = (pictureBehavior as any).getCapturedStaticTexture();
                                print(`InputNodeImage: Fallback - Using static camera frame (full frame, not cropped)`);
                            }
                        } catch (fallbackError) {
