...
export class InputNodeImage extends BaseScriptComponent {
    ...
    private setupCaptureListener(): void {
        ...
        if (!pictureBehavior) {
            print("InputNodeImage: WARNING - PictureBehavior not found in crop circle, cannot listen for capture");
            return;
        }
        ...
        this.createEvent("UpdateEvent").bind(() => {
            ...
            if (!this.cropCircleObject || isNull(this.cropCircleObject)) {
                callbackExecuted = true;
                return; // Crop circle was destroyed or hidden
            }

            checkCount++;
            if (checkCount > maxChecks) {
                print("InputNodeImage: Capture timeout - hiding crop circle");
                if (this.cropCircleObject) {
            ...
            // When captureImage is set, the screenCropTexture contains the cropped image
            try {
                ...
                if (captureRendMesh && captureRendMesh.mainPass && captureRendMesh.mainPass.captureImage) {
                    // Capture is complete - use screenCropTexture which contains the cropped image
                    if (screenCropTexture) {
                        print("InputNodeImage: Capture detected! Getting texture from screenCropTexture...");
                        callbackExecuted = true;
                        this.onCaptureComplete(screenCropTexture);
                        // Hide crop circle after capture
                        if (this.cropCircleObject) {
                            this.cropCircleObject.enabled = false;
                            print("InputNodeImage: Crop circle hidden after capture");
                        }
                    }
                }
            } catch (e) {
