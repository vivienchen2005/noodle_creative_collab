...
export class InputNode extends BaseScriptComponent {
    ...
    onAwake() {
        print(`InputNode: onAwake called, inputType="${this.inputType}"`);
        ...
        if (!this.baseNode) {
            ...
            print(`InputNode: Found BaseNode component`);
        } else {
            print(`InputNode: BaseNode already assigned`);
        }

        // Set node type based on input type
        if (this.baseNode) {
            this.baseNode.nodeType = this.inputType;
            print(`InputNode: Set BaseNode type to: ${this.inputType}`);
        }

        print(`InputNode: onAwake complete with input type: ${this.inputType}`);
        
        // Fallback: If onStart doesn't get called, try to initialize in UpdateEvent
        // This is a safety net in case onStart has issues
        let fallbackAttempts = 0;
        this.createEvent("UpdateEvent").bind(() => {
            // If onStart hasn't been called after a few frames, call it manually
            if (!this._onStartCalled && fallbackAttempts > 5) {
                print(`InputNode: WARNING - onStart not called after ${fallbackAttempts} frames, calling manually...`);
    ...
    onStart() {
        ...
        if (!this.baseNode) {
            print(`InputNode: ERROR - baseNode is null in onStart, trying to find it...`);
            this.baseNode = this.sceneObject.getComponent(BaseNode.getTypeName() as any) as BaseNode | null;
            if (!this.baseNode) {
                print(`InputNode: ERROR - Still cannot find BaseNode, setup will fail`);
                return;
            }
        }
        ...
        if (frame) {
            print(`InputNode: Frame is already available, attempting immediate setup...`);
            // Give it a frame to ensure everything is ready
            this.createEvent("UpdateEvent").bind(() => {
        ...
        const maxFrameCheckAttempts = 120; // Wait up to 2 seconds (120 frames at 60fps)
        ...
        this.createEvent("UpdateEvent").bind(() => {
            if (!this._initialized && this.baseNode) {
                ...
                // Log every 20 frames to show progress
                if (frameCheckAttempts % 20 === 0) {
                    print(`InputNode: Frame check attempt ${frameCheckAttempts}/${maxFrameCheckAttempts}, frame exists=${!!frame}`);
                }
                ...
                if (frameCheckAttempts >= maxFrameCheckAttempts && !this._initialized) {
                    print(`InputNode: WARNING - Frame check timeout after ${frameCheckAttempts} attempts, attempting setup anyway...`);
                    this.setupNode();
                    this._initialized = true;
                }
            }
    ...
    private setupNode(): void {
        print(`InputNode: setupNode() called`);

        if (!this.baseNode) {
            print(`InputNode: ERROR - baseNode is null, cannot setup node`);
            return;
        }

        const frame = this.baseNode.getFrame();
        if (!frame) {
            print(`InputNode: ERROR - Frame is null, cannot setup node`);
            return;
        }

        print(`InputNode: BaseNode and Frame are ready, starting UI setup...`);

        // Create title
        print(`InputNode: Setting up title...`);
        this.setupTitle();

        // Create output button
        print(`InputNode: Setting up output button...`);
        this.setupOutputButton();

        // Create content based on type
        print(`InputNode: Setting up content for type: ${this.inputType}`);
        ...
        // Update title text
        print(`InputNode: Updating title...`);
        this.updateTitle();

        print(`InputNode: setupNode() complete!`);
    }
