...
export class InputNode extends BaseScriptComponent {
    ...
    @input
    @hint("TextInputField for prompt type (will be created if not set)")
    @allowUndefined
    public textInputField: TextInputField | null = null;

    @input
    @hint("Voice to text component (for prompt type - separate script)")
    @allowUndefined
    public voiceToTextComponent: ScriptComponent | null = null;
    ...
    /**
     * Sets up prompt content (TextInputField + voice-to-text integration)
     */
    private setupPromptContent(): void {
        ...
        this.textInputField = this._contentObject.createComponent(TextInputField.getTypeName() as any) as TextInputField;
        if (this.textInputField) {
            this.textInputField.placeholderText = "Speak or type your prompt...";
            ...
            print("InputNode: Created prompt TextInputField");
        }

        // TODO: Integrate with voice-to-text component when available
        if (this.voiceToTextComponent) {
            // Connect voice-to-text to textInputField
            print("InputNode: Voice-to-text component found, will integrate when ready");
        }
    }
    ...
    public getPromptText(): string {
        if (this.inputType === "prompt" && this.textInputField) {
