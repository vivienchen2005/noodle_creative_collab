...
export class InputNodePrompt extends BaseScriptComponent {
    ...
    private findOrCreateConnectionController(): void {
        ...
        for (let i = 0; i < rootObjects; i++) {
            ...
            const controller = rootObject.getComponent(NodeConnectionController.getTypeName() as any) as NodeConnectionController | null;
            if (controller) {
        ...
        const controller = controllerObject.createComponent(NodeConnectionController.getTypeName() as any) as NodeConnectionController;
        if (controller) {
            print(`InputNodePrompt: NodeConnectionController created`);
            controller.onInputNodeButtonClicked(this.sceneObject, "text", this._outputButtonId);
        }
    }
