...
let BezierCurve = (() => {
    ...
    var BezierCurve = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.controlPointSpread = this.controlPointSpread;
        ...
        calculateControlPoints(start, end) {
            ...
            const t1 = spread;
            const point1AlongPath = new vec3(start.x + (end.x - start.x) * t1, start.y + (end.y - start.y) * t1, start.z + (end.z - start.z) * t1);
            this.controlPoint1 = point1AlongPath.add(offset);
            // Second control point: positioned partway from end (symmetric)
            const t2 = 1 - spread;
            const point2AlongPath = new vec3(start.x + (end.x - start.x) * t2, start.y + (end.y - start.y) * t2, start.z + (end.z - start.z) * t2);
