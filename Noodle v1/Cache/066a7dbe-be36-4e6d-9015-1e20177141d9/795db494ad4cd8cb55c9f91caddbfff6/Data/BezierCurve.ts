...
export class BezierCurve extends BaseScriptComponent {
    ...
    @input
    @hint("Curve direction - offset direction for the control point (0 = up, 1 = right, 2 = forward)")
    @widget(
        new ComboBoxWidget()
            .addItem("Up", 0)
            .addItem("Right", 1)
    ...
    private lastStartPosition: vec3 = new vec3();
    private lastEndPosition: vec3 = new vec3();
    private splinePoints: vec3[] = [];
    private middleControlPoint: vec3 = new vec3();
    ...
    private calculateMiddleControlPoint(start: vec3, end: vec3): void {
        ...
        this.middleControlPoint = midpoint.add(perpendicular.multiply(offsetDistance));
    }
