...
export class ConnectionLine extends BaseScriptComponent {
    ...
    onStart() {
        // Create BezierCurve component after ensuring all objects exist
        if (!this.bezierCurve && this.startPointObject && this.endPointObject && this.curveObject) {
            // Create BezierCurve component - it will validate inputs during creation
            // We need to ensure inputs are set, but Lens Studio validates before we can set them
            // Workaround: The validation error will occur, but we can handle it by ensuring
            // the SceneObjects exist and are valid before creation
            
            // Create the component - this will throw an error if inputs aren't set
            // But we'll set them immediately after in the same frame
            this.bezierCurve = this.curveObject.createComponent(BezierCurve.getTypeName() as any) as BezierCurve;
            
            // Immediately set the required inputs (even though validation already happened)
            if (this.bezierCurve) {
                // Set required inputs
