...
export class ConnectionLine extends BaseScriptComponent {
    ...
    onAwake() {
        ...
        // Create BezierCurve component on a child object
        const curveObject = global.scene.createSceneObject("BezierCurve");
        curveObject.setParent(this.sceneObject);
        
        // IMPORTANT: Set inputs BEFORE creating the component
        // We'll use a workaround: create the component, but delay its initialization
        // by setting the inputs immediately after creation but before onAwake completes
        
        // Actually, we need to set the inputs on the component after creation
        // But Lens Studio validates required inputs during component creation
        // So we'll create the component with the inputs set via a different approach
        
        // Create component and immediately set required inputs
        this.bezierCurve = curveObject.createComponent(BezierCurve.getTypeName() as any) as BezierCurve;
        
        // Set required inputs immediately (before onAwake completes)
        if (this.bezierCurve) {
            // Set start and end points IMMEDIATELY
            (this.bezierCurve as any).startPoint = this.startPointObject;
            (this.bezierCurve as any).endPoint = this.endPointObject;
            ...
            this.bezierCurve.isEnabled = false; // Disable initially until we have valid positions
        }
    }
