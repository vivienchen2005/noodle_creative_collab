...
let ConnectionLine = (() => {
    ...
    var ConnectionLine = _classThis = class extends _classSuper {
        ...
        onAwake() {
            ...
            // Create the component - onAwake will be called immediately
            // We need to set the inputs BEFORE onAwake runs, but we can't do that
            // So we'll delay the BezierCurve creation to onStart instead
            this.bezierCurve = null;
        }
        onStart() {
            // Delay BezierCurve creation by one frame to ensure inputs are ready
            let frameCount = 0;
            this.createEvent("UpdateEvent").bind(() => {
                // Create BezierCurve on first update frame
                if (frameCount === 0 && !this.bezierCurve && this.startPointObject && this.endPointObject) {
                    ...
                    this.bezierCurve = targetObject.createComponent(BezierCurve_1.BezierCurve.getTypeName());
                    if (this.bezierCurve) {
                        // Set required inputs immediately after creation
                        // Note: onAwake will be called, but we set inputs right after
