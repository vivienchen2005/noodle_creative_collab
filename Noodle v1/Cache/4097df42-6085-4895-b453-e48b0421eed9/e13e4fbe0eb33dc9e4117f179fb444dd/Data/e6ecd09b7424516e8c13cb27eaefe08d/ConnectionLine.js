...
let ConnectionLine = (() => {
    ...
    var ConnectionLine = _classThis = class extends _classSuper {
        ...
        initializeBezierCurve() {
            ...
            // BezierCurve requires startPoint and endPoint to be set DURING component creation
            // We need to create temporary placeholders first, then update to real objects
            const tempStart = global.scene.createSceneObject("TempStart");
            const tempEnd = global.scene.createSceneObject("TempEnd");
            tempStart.setParent(this.bezierCurveObject);
            tempEnd.setParent(this.bezierCurveObject);
            // Position them at the same location as the real start/end points
            tempStart.getTransform().setWorldPosition(this.startPoint.getTransform().getWorldPosition());
            tempEnd.getTransform().setWorldPosition(this.endPoint.getTransform().getWorldPosition());
            // Create the BezierCurve component (this will trigger validation)
            this.bezierCurve = this.bezierCurveObject.createComponent(BezierCurve_1.BezierCurve.getTypeName());
            // Immediately set the temporary placeholders (required for validation)
            this.bezierCurve.startPoint = tempStart;
            this.bezierCurve.endPoint = tempEnd;
            ...
            // Wait a frame, then update to real objects and clean up
            this.createEvent("UpdateEvent").bind(() => {
                if (this.bezierCurve && this.startPoint && this.endPoint) {
