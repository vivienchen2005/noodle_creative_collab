      ...
      try {
        ...
        if (cropControl && cropControl.inputTexture) {
          ...
          if ((camTexture as any).copyFrame && typeof (camTexture as any).copyFrame === 'function') {
            ...
            // Verify crop region is set before capturing
            if (cropControl.cropRect) {
              const cropRect = cropControl.cropRect
              print(`[PictureBehavior] Crop region: center=(${cropRect.getCenter().x.toFixed(2)}, ${cropRect.getCenter().y.toFixed(2)}), size=(${cropRect.getSize().x.toFixed(2)}, ${cropRect.getSize().y.toFixed(2)})`)
            } else {
              print("[PictureBehavior] WARNING - Crop region not set! The cropped texture will be the full frame.")
            }

            // Set static frame as input to screenCropTexture
            // The crop region should already be set by CropRegion, so screenCropTexture will crop the static frame
            cropControl.inputTexture = staticCamFrame
            print("[PictureBehavior] Set static camera frame as input to screenCropTexture (crop region should be applied)")

            // Wait for screenCropTexture to process the static frame with crop applied
            // Then create ProceduralTextureProvider to capture the cropped result
            let frameCount = 0
            const processEvent = this.createEvent("UpdateEvent")
            processEvent.bind(() => {
              frameCount++

              // Wait 2 frames for screenCropTexture to process the static input with crop
              if (frameCount >= 2) {
                try {
                  // Now screenCropTexture has processed the static frame with crop region applied
                  // Create ProceduralTextureProvider from screenCropTexture NOW (while static input is set)
                  // This should capture the static CROPPED result (only the circle area, not full frame)
                  ...
                  print("[PictureBehavior] This should show only the area inside the circle, not the full camera frame")
                } catch (e) {
