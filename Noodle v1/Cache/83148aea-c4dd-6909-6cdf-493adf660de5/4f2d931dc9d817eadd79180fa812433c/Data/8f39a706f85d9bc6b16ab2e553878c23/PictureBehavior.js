...
let PictureBehavior = (() => {
    ...
    var PictureBehavior = _classThis = class extends _classSuper {
        ...
        captureAndProcessImage() {
            ...
            // Use the simple approach from the reference: directly create ProceduralTextureProvider from screenCropTexture
            // screenCropTexture already has the crop region set by CropRegion, so it will capture the cropped area
            if (this.screenCropTexture && this.screenCropTexture.getColorspace() == 3) {
                try {
                    ...
                    // Directly create ProceduralTextureProvider from screenCropTexture
                    // This captures the current state of screenCropTexture, which has the crop region applied
                    // The reference implementation shows this is the correct approach
                    this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(this.screenCropTexture);
                    print("[PictureBehavior] CROPPED texture captured successfully (using reference approach)");
