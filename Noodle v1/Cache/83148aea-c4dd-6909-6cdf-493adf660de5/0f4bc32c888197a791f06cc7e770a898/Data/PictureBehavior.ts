...
export class PictureBehavior extends BaseScriptComponent {
  ...
  onAwake() {
    ...
    this._renderTargetObject = global.scene.createSceneObject("HiddenRenderTarget")
    this._renderTargetObject.setParent(this.getSceneObject())
    this._renderTargetObject.enabled = false // Keep it hidden
    
    // Create RenderTarget texture
    this._renderTarget = global.scene.createRenderTarget("StaticCaptureRT", 1024, 1024)
    if (this._renderTarget) {
      // Create camera to render to the RenderTarget
      this._renderTargetCamera = this._renderTargetObject.createComponent("Component.Camera") as Camera
      if (this._renderTargetCamera) {
        this._renderTargetCamera.renderTarget = this._renderTarget
        this._renderTargetCamera.enabled = false // Keep it disabled until we need to capture
        print("[PictureBehavior] Created RenderTarget and Camera for static texture capture")
      }
    }
  ...
  private captureAndProcessImage() {
    ...
    // We use a RenderTarget to render the cropped texture to a static snapshot
    if (this.screenCropTexture && this.screenCropTexture.getColorspace() == 3) {
      try {
        // Use RenderTarget to capture static cropped texture
        if (this._renderTarget && this._renderTargetCamera && this._hiddenCaptureImage && this._hiddenCaptureObject) {
          // Set the cropped texture to the hidden image
          if (this._hiddenCaptureImage.mainPass) {
            this._hiddenCaptureImage.mainPass.baseTex = this.screenCropTexture
            
            // Position the hidden image in front of the render camera
            const imageTransform = this._hiddenCaptureObject.getTransform()
            imageTransform.setLocalPosition(new vec3(0, 0, -1))
            imageTransform.setLocalScale(new vec3(2, 2, 1))
            
            // Enable the hidden image and camera temporarily to render to RenderTarget
            this._hiddenCaptureObject.enabled = true
            this._renderTargetCamera.enabled = true
            this._renderTargetObject.enabled = true

            // Wait for rendering to complete, then get the static texture from RenderTarget
            let frameCount = 0
            const captureFrameEvent = this.createEvent("UpdateEvent")
            captureFrameEvent.bind(() => {
              frameCount++

              // Wait 3 frames to ensure the RenderTarget is rendered
              if (frameCount >= 3) {
                try {
                  // The RenderTarget now contains the static cropped image
                  // Copy it to get a truly static texture
                  if (this._renderTarget && (this._renderTarget as any).copyFrame && typeof (this._renderTarget as any).copyFrame === 'function') {
                    this._capturedStaticTexture = (this._renderTarget as any).copyFrame()
                    print("[PictureBehavior] Copied static frame from RenderTarget")
                  } else {
                    // Use RenderTarget directly (should be static)
                    this._capturedStaticTexture = this._renderTarget
                    print("[PictureBehavior] Using RenderTarget directly as static texture")
                  }

                  // Set captureImage for detection
                  this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(this.screenCropTexture)

                  // Disable hidden objects again
                  this._hiddenCaptureObject.enabled = false
                  this._renderTargetCamera.enabled = false
                  this._renderTargetObject.enabled = false

                  print("[PictureBehavior] Static texture captured from RenderTarget")
                } catch (copyError) {
                  print("[PictureBehavior] Error getting texture from RenderTarget: " + copyError)
                ...
                // Remove this event
                this.removeEvent(captureFrameEvent)
              }
            })
          } else {
            print("[PictureBehavior] Hidden image has no mainPass")
            this._capturedStaticTexture = null
            this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(this.screenCropTexture)
          }
        } else {
          print("[PictureBehavior] RenderTarget or hidden image not created, using fallback")
          this._capturedStaticTexture = null
          this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(this.screenCropTexture)
        }
      } catch (e) {
        print("[PictureBehavior] Error capturing frame: " + e)
        if (this._hiddenCaptureObject) {
          this._hiddenCaptureObject.enabled = false
        }
        if (this._renderTargetCamera) {
          this._renderTargetCamera.enabled = false
        }
        if (this._renderTargetObject) {
          this._renderTargetObject.enabled = false
        }
        return
      }
