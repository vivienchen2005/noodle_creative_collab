...
export class PictureBehavior extends BaseScriptComponent {
  ...
  private captureAndProcessImage() {
    ...
    // We copy the camera frame (static) and use screenCropTexture's control to get cropped version
    // The key is: we need to copy the frame FIRST, then apply crop to the static copy
    if (this.screenCropTexture && this.screenCropTexture.getColorspace() == 3) {
      try {
        ...
        if (cropControl && cropControl.inputTexture) {
          ...
          // Copy the frame to get a static snapshot of the camera
          if ((camTexture as any).copyFrame && typeof (camTexture as any).copyFrame === 'function') {
            const staticCamFrame = (camTexture as any).copyFrame()
            print("[PictureBehavior] Copied static camera frame")

            // Now we need to apply the crop to this static frame
            // The screenCropTexture has the crop region set by CropRegion
            // We can temporarily set the static frame as input, wait for processing, then get the result
            const originalInput = cropControl.inputTexture

            // Set static frame as input to screenCropTexture
            cropControl.inputTexture = staticCamFrame

            // Wait for screenCropTexture to process the static frame
            // Then use the hidden Image component to render it and get a static snapshot
            let frameCount = 0
            const processEvent = this.createEvent("UpdateEvent")
            processEvent.bind(() => {
              frameCount++

              // Wait 2 frames for screenCropTexture to process the static input
              if (frameCount >= 2) {
                try {
                  // Now screenCropTexture has processed the static frame with crop applied
                  // Use the hidden Image component to render it
                  if (this._hiddenCaptureImage && this._hiddenCaptureImage.mainPass && this._hiddenCaptureObject) {
                    // Enable hidden image temporarily to render
                    this._hiddenCaptureObject.enabled = true

                    // Set the processed screenCropTexture to the hidden image
                    this._hiddenCaptureImage.mainPass.baseTex = this.screenCropTexture

                    // Wait one more frame for the image to render
                    let renderFrameCount = 0
                    const renderEvent = this.createEvent("UpdateEvent")
                    renderEvent.bind(() => {
                      renderFrameCount++

                      if (renderFrameCount >= 1) {
                        // The hidden image has now rendered the cropped static texture
                        // screenCropTexture (with static input) has the cropped result
                        // We need to get a static copy of this cropped texture

                        // The renderedTexture is screenCropTexture, which is still a live provider
                        // But since we set a static frame as input, the output should be static
                        // Create ProceduralTextureProvider from screenCropTexture NOW (while static input is set)
                        // This should capture the static cropped result
                        this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(this.screenCropTexture)

                        // Store the static camera frame (full frame, not cropped)
                        // InputNodeImage will use captureImage which has the cropped static result
                        this._capturedStaticTexture = staticCamFrame

                        // Restore original input AFTER creating the ProceduralTextureProvider
                        // This ensures the ProceduralTextureProvider captured the static cropped version
                        cropControl.inputTexture = originalInput

                        // Disable hidden image
                        this._hiddenCaptureObject.enabled = false

                        print("[PictureBehavior] Static CROPPED texture captured (created ProceduralTextureProvider while static input was set)")

                        // Remove render event
                        this.removeEvent(renderEvent)
                      }
                    })
                  } else {
                    // Fallback: use ProceduralTextureProvider directly
                    this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(this.screenCropTexture)
                    this._capturedStaticTexture = staticCamFrame
                    cropControl.inputTexture = originalInput
                    print("[PictureBehavior] Static cropped texture captured via ProceduralTextureProvider (fallback)")
                  }
                } catch (e) {
                  print("[PictureBehavior] Error processing static frame: " + e)
                  cropControl.inputTexture = originalInput
                ...
                // Remove this event
                this.removeEvent(processEvent)
              }
            })
          } else {
            print("[PictureBehavior] Camera texture has no copyFrame method")
            this._capturedStaticTexture = null
            this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(this.screenCropTexture)
          }
        } else {
