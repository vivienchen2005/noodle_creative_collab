...
export class PictureBehavior extends BaseScriptComponent {
  ...
  @input
  @allowUndefined
  @hint("Optional: SceneObject to show at pinch location for visual feedback. Will be positioned between thumb and index tip when pinching.")
  pinchVisualIndicator: SceneObject | null = null
  ...
  onAwake() {
    ...
    // Try to find pinch visual indicator if not assigned
    if (!this.pinchVisualIndicator) {
      this.pinchVisualIndicator = this.findPinchVisualIndicator()
      if (this.pinchVisualIndicator) {
        print("[PictureBehavior] Found pinch visual indicator automatically: " + this.pinchVisualIndicator.name)
      }
    }

    // Hide pinch visual indicator initially - force it to be disabled
    if (this.pinchVisualIndicator) {
      this.pinchVisualIndicator.enabled = false
      print("[PictureBehavior] Pinch visual indicator hidden initially (enabled: " + this.pinchVisualIndicator.enabled + ")")
    } else {
      print("[PictureBehavior] No pinch visual indicator assigned or found")
    }
  }

  /**
   * Tries to find the pinch visual indicator in the scene hierarchy
   * Looks for common names like "PinchIndicator", "PinchVisual", etc.
   */
  private findPinchVisualIndicator(): SceneObject | null {
    // Search in the current scene object's hierarchy first
    const currentObj = this.getSceneObject()

    // Check children
    for (let i = 0; i < currentObj.getChildrenCount(); i++) {
      const child = currentObj.getChild(i)
      const name = child.name.toLowerCase()
      if (name.includes("pinch") && (name.includes("indicator") || name.includes("visual"))) {
        return child
      }
    }

    // Search in parent
    const parent = currentObj.getParent()
    if (parent) {
    ...
    for (let i = 0; i < rootObjects; i++) {
      ...
      const found = this.searchForPinchIndicator(rootObject)
      if (found) {
        return found
      }
    }
  ...
  /**
   * Recursively searches for pinch indicator in a scene object hierarchy
   */
  private searchForPinchIndicator(obj: SceneObject): SceneObject | null {
    ...
    for (let i = 0; i < obj.getChildrenCount(); i++) {
      ...
      const found = this.searchForPinchIndicator(child)
      if (found) {
        return found
      }
    }
  ...
  private startTracking() {
    ...
    // Don't manually show here - let updatePinchVisualIndicator() handle it
    // This ensures consistency with the update loop
  ...
  private rightPinchUp = () => {
    ...
    // Force hide pinch visual indicator immediately
    if (this.pinchVisualIndicator) {
      this.pinchVisualIndicator.enabled = false
      print("[PictureBehavior] Pinch indicator force-hidden on pinch up")
    }
  ...
  /**
   * Updates pinch visual indicator visibility based on pinch state
   * Shows only when actively pinching, hides otherwise
   * The object is already positioned under the hand, so we just control visibility
   */
  private updatePinchVisualIndicator(): void {
    if (!this.pinchVisualIndicator) {
      return
    }

    // Force visibility based on isPinching state
    // Always explicitly set it to ensure it's controlled properly
    const shouldBeVisible = this.isPinching

    if (this.pinchVisualIndicator.enabled !== shouldBeVisible) {
      this.pinchVisualIndicator.enabled = shouldBeVisible
      if (shouldBeVisible) {
        print("[PictureBehavior] Pinch indicator SHOWN (isPinching: " + this.isPinching + ")")
      } else {
        print("[PictureBehavior] Pinch indicator HIDDEN (isPinching: " + this.isPinching + ")")
      }
    }
  }
