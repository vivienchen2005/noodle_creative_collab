      ...
      try {
        ...
        if (cropControl && cropControl.inputTexture) {
          ...
          if ((camTexture as any).copyFrame && typeof (camTexture as any).copyFrame === 'function') {
            ...
            // Temporarily set the static frame as input to screenCropTexture
            // This will make screenCropTexture output a cropped version of the static frame
            const originalInput = cropControl.inputTexture
            cropControl.inputTexture = staticCamFrame

            // Wait a frame for screenCropTexture to process the static frame
            // Then screenCropTexture will contain the cropped static texture
            // We'll use screenCropTexture itself as the static texture (it's now processing a static input)
            // Actually, screenCropTexture is still a live provider, so we need a different approach

            // Store the static camera frame - InputNodeImage can use it with the crop region
            // Or we can create a ProceduralTextureProvider from screenCropTexture at this moment
            this._capturedStaticTexture = staticCamFrame

            // Create a ProceduralTextureProvider from screenCropTexture NOW (while it has static input)
            // This should capture the cropped static texture
            this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(this.screenCropTexture)

            // Restore original input after a frame
            const restoreEvent = this.createEvent("DelayedCallbackEvent")
            restoreEvent.bind(() => {
              cropControl.inputTexture = originalInput
            })
            restoreEvent.reset(0.1) // Wait 100ms for processing

            print("[PictureBehavior] Static frame captured and cropped - stored in _capturedStaticTexture")
          } else {
