...
export class PictureBehavior extends BaseScriptComponent {
  ...
  onAwake() {
    ...
    if (this.pinchVisualIndicator) {
      try {
        ...
        print("[PictureBehavior] Pinch visual indicator hidden initially (enabled: " + this.pinchVisualIndicator.enabled + ", name: " + this.pinchVisualIndicator.name + ")")

        // Also check if parent is enabled
        const parent = this.pinchVisualIndicator.getParent()
        if (parent) {
          print("[PictureBehavior] Pinch indicator parent: " + parent.name + " (enabled: " + parent.enabled + ")")
        }
      } catch (e) {
        print("[PictureBehavior] Error hiding pinch indicator initially: " + e)
      }
    } else {
  ...
  private startTracking() {
    ...
    if (this.pinchVisualIndicator) {
      try {
        ...
        print("[PictureBehavior] Pinch indicator shown (enabled: " + this.pinchVisualIndicator.enabled + ")")
      } catch (e) {
        print("[PictureBehavior] Error showing pinch indicator: " + e)
      }
    } else {
      print("[PictureBehavior] WARNING: pinchVisualIndicator is null, cannot show")
    }
  ...
  private rightPinchUp = () => {
    ...
    if (this.pinchVisualIndicator) {
      try {
        this.pinchVisualIndicator.enabled = false
        print("[PictureBehavior] Pinch indicator hidden on pinch up (enabled: " + this.pinchVisualIndicator.enabled + ")")
      } catch (e) {
        print("[PictureBehavior] Error hiding pinch indicator: " + e)
      }
    } else {
      print("[PictureBehavior] WARNING: pinchVisualIndicator is null, cannot hide")
    }
