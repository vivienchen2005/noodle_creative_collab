...
export class PictureBehavior extends BaseScriptComponent {
  ...
  private captureAndProcessImage() {
    ...
    // Use the EXACT approach from the reference implementation (line 174 in reference)
    // Simply create ProceduralTextureProvider from screenCropTexture - it already has crop region applied
    if (this.screenCropTexture && this.screenCropTexture.getColorspace() == 3) {
      try {
        // Verify crop region is set (CropRegion should have set it via update loop)
        ...
        if (cropControl && cropControl.cropRect) {
          ...
          print(`[PictureBehavior] Crop region is set: center=(${center.x.toFixed(2)}, ${center.y.toFixed(2)}), size=(${size.x.toFixed(2)}, ${size.y.toFixed(2)})`)
        } else {
          print("[PictureBehavior] WARNING - Crop region not found! This might result in full frame capture.")
        }

        // This is the exact line from the reference - it works there, so it should work here
        // screenCropTexture already has the crop region applied by CropRegion component
        // CropRegion.update() continuously updates the crop region based on circle positions
        this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(this.screenCropTexture)
        print("[PictureBehavior] Frame captured successfully (using reference approach)")
        print("[PictureBehavior] The cropped region from the circle gesture is now available in captureImage")

        // Disable crop region after capture (like in reference line 201)
        if (this.cropRegion) {
          this.cropRegion.enabled = false
          print("[PictureBehavior] Crop region disabled after capture")
        }
      } catch (e) {
