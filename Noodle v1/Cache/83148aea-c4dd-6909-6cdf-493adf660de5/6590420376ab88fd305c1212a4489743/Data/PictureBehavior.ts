...
export class PictureBehavior extends BaseScriptComponent {
  ...
  private captureAndProcessImage() {
    ...
    // We use a hidden Image component to render the cropped texture, then copy it to get a static snapshot
    if (this.screenCropTexture && this.screenCropTexture.getColorspace() == 3) {
      try {
        // Use hidden Image component to capture static texture
        if (this._hiddenCaptureImage && this._hiddenCaptureObject) {
          // Enable the hidden image temporarily (it's still hidden from view)
          ...
          if (this._hiddenCaptureImage.mainPass) {
            ...
            // Wait a frame for it to render, then copy the frame to get static texture
            let frameCount = 0
            const captureFrameEvent = this.createEvent("UpdateEvent")
            captureFrameEvent.bind(() => {
              frameCount++

              // Wait 2 frames to ensure the image is rendered
              if (frameCount >= 2) {
                // Try to copy the frame from the hidden image
                // The image component's baseTex should now have the rendered cropped texture
                // We can use it directly, or try to copy it
                try {
                  // Get the texture from the image (it's now rendered)
                  const renderedTexture = this._hiddenCaptureImage.mainPass.baseTex

                  // Try to copy the frame if it's a camera texture
                  if (renderedTexture && (renderedTexture as any).copyFrame && typeof (renderedTexture as any).copyFrame === 'function') {
                    this._capturedStaticTexture = (renderedTexture as any).copyFrame()
                    print("[PictureBehavior] Copied static frame from hidden image")
                  } else {
                    // If we can't copy, use the texture directly (it should be static from screenCropTexture at capture time)
                    // Actually, screenCropTexture is still live, so we need to use ProceduralTextureProvider
                    this._capturedStaticTexture = renderedTexture
                    print("[PictureBehavior] Using rendered texture directly (may still be live)")
                  }
                  ...
                  // Disable hidden image again
                  this._hiddenCaptureObject.enabled = false

                  print("[PictureBehavior] Static texture captured from hidden image")
                } catch (copyError) {
                  print("[PictureBehavior] Error copying frame: " + copyError)
          ...
          print("[PictureBehavior] Hidden capture image not created, using fallback")
          this._capturedStaticTexture = null
          this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(this.screenCropTexture)
        }
      } catch (e) {
