...
export class PictureBehavior extends BaseScriptComponent {
   ...
   * Positions it at the midpoint between thumb and index tip when pinching
   * Hides it when not pinching
   */
  private updatePinchVisualIndicator(): void {
    ...
    try {
      ...
      // Only show and position if actively pinching (distance < 3cm and isPinching is true)
      if (distance < 3 && this.isPinching) {
        ...
        // Hide when not pinching
        this.pinchVisualIndicator.enabled = false
      }
    } catch (e) {
      // Hand tracking might not be available yet, hide indicator and ignore errors
      if (this.pinchVisualIndicator) {
        this.pinchVisualIndicator.enabled = false
      }
    }
