...
let PictureBehavior = (() => {
    ...
    var PictureBehavior = _classThis = class extends _classSuper {
        ...
        captureAndProcessImage() {
            ...
            // Note: We set captureImage for compatibility, but the actual cropped texture is in screenCropTexture
            // InputNodeImage will use screenCropTexture directly, not captureImage
            if (this.screenCropTexture && this.screenCropTexture.getColorspace() == 3) {
                try {
                    // Set captureImage for detection purposes, but visual elements are already hidden
                    this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(this.screenCropTexture);
                    print("[PictureBehavior] Frame captured successfully - screenCropTexture ready for InputNodeImage");
                }
                catch (e) {
            ...
            if (this.updateEvent != null) {
                ...
                // HIDE visual elements - don't display captured image in 3D space
                // The texture will be passed to InputNodeImage instead
                if (this.picAnchorObj) {
                    this.picAnchorObj.enabled = false;
                    print("[PictureBehavior] Hidden picAnchorObj - image will not be displayed in 3D space");
                }
                if (this.captureRendMesh && this.captureRendMesh.sceneObject) {
                    this.captureRendMesh.sceneObject.enabled = false;
                    print("[PictureBehavior] Hidden captureRendMesh - image will not be displayed in 3D space");
                }
                if (this.loadingObj) {
                    this.loadingObj.enabled = false;
                    print("[PictureBehavior] Hidden loadingObj");
                }
                // Frame is now captured and available in captureRendMesh.mainPass.captureImage
                // The texture (screenCropTexture) is available for InputNodeImage to use
                // Visual elements are hidden so image is NOT displayed in 3D space
                print("[PictureBehavior] Freeze frame captured and ready to use! (Visual display disabled)");
            }
        }
        localTopLeft() {
