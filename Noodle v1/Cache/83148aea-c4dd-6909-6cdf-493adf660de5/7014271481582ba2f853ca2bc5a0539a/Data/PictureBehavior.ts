...
export class PictureBehavior extends BaseScriptComponent {
  ...
  // Visual feedback for pinch drawing
  private pinchIndicator: SceneObject | null = null
  private pinchGlow: RenderMeshVisual | null = null
  private pinchTrail: SceneObject[] = [] // Trail of visual indicators following the drawing
  ...
  private startTracking() {
    ...
    // Create visual feedback for pinch drawing
    this.createPinchVisualFeedback()
  ...
  /**
   * Creates visual feedback to show the user is pinching to draw
   */
  private createPinchVisualFeedback(): void {
    try {
      // Create a glow indicator at the thumb tip position
      if (!this.pinchIndicator) {
        this.pinchIndicator = global.scene.createSceneObject("PinchIndicator")
        this.pinchIndicator.setParent(this.getSceneObject())
        
        // Create a small sphere or glow effect
        this.pinchGlow = this.pinchIndicator.createComponent("RenderMeshVisual") as RenderMeshVisual
        if (this.pinchGlow) {
          // Use a unit sphere mesh (you may need to assign a mesh in Lens Studio)
          // For now, we'll just position it and make it visible
          this.pinchIndicator.getTransform().setWorldScale(new vec3(0.5, 0.5, 0.5))
          print("[PictureBehavior] Created pinch visual indicator")
        }
      }
      
      if (this.pinchIndicator) {
        this.pinchIndicator.enabled = true
      }
    } catch (e) {
      print("[PictureBehavior] Could not create pinch visual feedback: " + e)
    }
  }

  /**
   * Removes visual feedback when pinch ends
   */
  private removePinchVisualFeedback(): void {
    if (this.pinchIndicator) {
      this.pinchIndicator.enabled = false
    }
    
    // Clean up trail
    for (let i = 0; i < this.pinchTrail.length; i++) {
      if (this.pinchTrail[i]) {
        this.pinchTrail[i].destroy()
      }
    }
    this.pinchTrail = []
  }
  ...
  private rightPinchUp = () => {
    ...
    this.removePinchVisualFeedback()
  ...
  update() {
    if (this.isPinching) {
      ...
      // Update visual feedback position to follow thumb tip
      if (this.pinchIndicator) {
        // Position indicator at thumb tip
        this.pinchIndicator.getTransform().setWorldPosition(currentPos)
        
        // Also show index tip position for better feedback
        try {
          const indexPos = this.rightHand.indexTip.position
          const pinchDistance = currentPos.distance(indexPos)
          
          // Scale the indicator based on pinch distance (closer = smaller, further = larger)
          const scale = Math.max(0.3, Math.min(1.0, pinchDistance / 5.0))
          this.pinchIndicator.getTransform().setWorldScale(new vec3(scale, scale, scale))
        } catch (e) {
          // Hand data might not be available
        }
      }
