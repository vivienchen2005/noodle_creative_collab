...
export class PictureBehavior extends BaseScriptComponent {
  ...
  private initializePinchVisualization(): void {
    try {
      let rightHandMeshObj: SceneObject | null = null

      // First, try to use the input reference if provided
      if (this.rightHandMeshObject) {
        rightHandMeshObj = this.rightHandMeshObject
        print("[PictureBehavior] Using provided rightHandMeshObject input")
      } else {
        // Fallback: search for hand mesh automatically
        print("[PictureBehavior] No rightHandMeshObject input provided, searching automatically...")
        rightHandMeshObj = this.findSceneObjectByName(null, "RightHandMeshIndexThumb")
      ...
      // Get RenderMeshVisual from the found/assigned mesh object
      if (rightHandMeshObj) {
        ...
        if (this.rightHandMesh) {
          ...
          if (materialCount > 1) {
            ...
            if (originalMaterial) {
              // Clone the material and replace it (similar to InteractionHintController)
              this.rightHandTipGlowMaterial = originalMaterial.clone()
              // Store other materials
              const material0 = this.rightHandMesh.getMaterial(0)
              const material2 = materialCount > 2 ? this.rightHandMesh.getMaterial(2) : null

              // Clear and re-add materials with cloned glow material
              this.rightHandMesh.clearMaterials()
              if (material0) this.rightHandMesh.addMaterial(material0)
              this.rightHandMesh.addMaterial(this.rightHandTipGlowMaterial)
              if (material2) this.rightHandMesh.addMaterial(material2)
              ...
              print("[PictureBehavior] Could not get material at index 1")
            }
          } else {
            print("[PictureBehavior] Hand mesh doesn't have enough materials for glow effect (needs at least 2)")
          }
        } else {
          print("[PictureBehavior] Could not find RenderMeshVisual component on right hand mesh object")
        }
      } else {
        print("[PictureBehavior] Right hand mesh object not found")
      }
    } catch (e) {
  ...
  private findHandVisualComponent(): SceneObject | null {
    ...
    for (let i = 0; i < rootObjects; i++) {
      ...
      // Use type casting for custom component types
      const handVisual = rootObject.getComponent("HandVisual" as any)
      if (handVisual) {
      ...
      for (let j = 0; j < childCount; j++) {
        ...
        const childHandVisual = child.getComponent("HandVisual" as any)
        if (childHandVisual) {
  ...
  private findHandMeshAlternative(): void {
    ...
    if (handMesh) {
      ...
      if (this.rightHandMesh && this.rightHandMesh.getMaterialsCount() > 1) {
        const originalMaterial = this.rightHandMesh.getMaterial(1)
        if (originalMaterial) {
          // Clone and replace material (similar to InteractionHintController)
          this.rightHandTipGlowMaterial = originalMaterial.clone()
          const material0 = this.rightHandMesh.getMaterial(0)
          const materialCount = this.rightHandMesh.getMaterialsCount()
          const material2 = materialCount > 2 ? this.rightHandMesh.getMaterial(2) : null
