                  ...
                  if (this._hiddenCaptureImage && this._hiddenCaptureImage.mainPass && this._hiddenCaptureObject) {
                    ...
                    renderEvent.bind(() => {
                      ...
                      if (renderFrameCount >= 1) {
                        ...
                        // The texture in the image should be the rendered result
                        // Use it directly - it should be static since the input was static
                        const renderedTexture = this._hiddenCaptureImage.mainPass.baseTex
                        
                        // Create ProceduralTextureProvider from the rendered texture
                        // This should be the static cropped result
                        this.captureRendMesh.mainPass.captureImage = ProceduralTextureProvider.createFromTexture(renderedTexture)
                        
                        // Store the static camera frame (InputNodeImage can use it if needed)
                        ...
                        print("[PictureBehavior] Static cropped texture captured via hidden image rendering")
