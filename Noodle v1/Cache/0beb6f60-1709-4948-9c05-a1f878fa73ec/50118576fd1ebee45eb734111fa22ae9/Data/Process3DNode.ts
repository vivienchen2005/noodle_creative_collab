...
export class Process3DNode extends BaseScriptComponent {
    ...
    private setupGenerateButton(): void {
        ...
        if (this.generateButton) {
            // RectangleButton size is set via transform scale (in meters)
            // Default size is 6x3x3 cm, so we'll use that scale
            const transform = this._generateButtonObject.getTransform();
            transform.setLocalScale(new vec3(1, 1, 1)); // Use default size from component
