...
const RemoteServiceGatewayCredentials_2 = require("RemoteServiceGateway.lspkg/RemoteServiceGatewayCredentials");
/**
...
let ProcessImageGenNode = (() => {
    ...
    var ProcessImageGenNode = _classThis = class extends _classSuper {
        constructor() {
            ...
            this._clonedOutputMaterial = null;
        ...
        connectTextInput(sourceNode, sourceButtonId = "") {
            ...
            this._textConnections.set(connectionId, {
                ...
                curve: null, // Will be set when ConnectionLine is created
         ...
         * Supports chaining: InputNodeImage OR another ProcessImageGenNode can be a source
         */
        connectImageInput(sourceNode, sourceButtonId = "") {
            // Check if source is InputNodeImage
            const imageInputNode = sourceNode.getComponent(InputNodeImage_1.InputNodeImage.getTypeName());
            // Check if source is another ProcessImageGenNode (chaining!)
            const imageGenNode = sourceNode.getComponent(ProcessImageGenNode.getTypeName());
            // Determine source type and get necessary info
            let outputButtonId = "";
            let outputButtonObject = null;
            let sourceType = "";
            if (imageInputNode) {
                ...
                if (!imageGenNode.hasGeneratedImage()) {
                    print(`ProcessImageGenNode: Source ProcessImageGenNode has no generated image yet - generate first!`);
                    return false;
                }
                ...
                print(`ProcessImageGenNode: Source node is not an InputNodeImage or ProcessImageGenNode`);
                return false;
            }
            // Generate unique connection ID
            ...
            // Update generate button state
            this.updateGenerateButtonState();
            print(`ProcessImageGenNode: Image input connected from ${sourceNode.name} (${sourceType}) (Connection ID: ${connectionId}, Total: ${this._imageConnections.size})`);
            return true;
        }
        /**
         ...
         */
        createImageConnectionFromButton(sourceNode, outputButtonObj, connectionId) {
            if (!this._imageInputSectionObject) {
            ...
            try {
                ...
                if (connectionLine) {
                    ...
                    print(`ProcessImageGenNode: Image ConnectionLine created for ${connectionId}`);
                    return connectionLine;
                }
            }
            catch (error) {
                print(`ProcessImageGenNode: ERROR - Failed to create image ConnectionLine: ${error}`);
            }
            return null;
        }
        ...
        getImageInputData() {
            ...
            // Check if source is InputNodeImage
            const imageInputNode = this._connectedImageNode.getComponent(InputNodeImage_1.InputNodeImage.getTypeName());
            if (imageInputNode) {
            ...
            const imageGenNode = this._connectedImageNode.getComponent(ProcessImageGenNode.getTypeName());
            if (imageGenNode) {
         ...
         */
        checkCredentials() {
            const googleToken = RemoteServiceGatewayCredentials_1.RemoteServiceGatewayCredentials.getApiToken(RemoteServiceGatewayCredentials_2.AvaliableApiTypes.Google);
        ...
        // ========== OUTPUT CONNECTION METHODS (for chaining to other nodes) ==========
        /**
         * Adds a connected child node (called when connection is made)
         * This node's output can connect to multiple process nodes
         */
        addChildNode(childNode) {
            if (!this._connectedChildNodes.includes(childNode)) {
                this._connectedChildNodes.push(childNode);
                print(`ProcessImageGenNode: Added child node: ${childNode.name} (Total: ${this._connectedChildNodes.length})`);
            }
        }
        /**
        ...
        removeChildNode(childNode) {
            ...
            if (index > -1) {
                ...
                print(`ProcessImageGenNode: Removed child node: ${childNode.name} (Total: ${this._connectedChildNodes.length})`);
            }
        }
        /**
        ...
        /**
         * Gets the generated image texture
         */
        getImageTexture() {
         ...
         */
        getClonedMaterial() {
            return this._clonedOutputMaterial;
        }
        /**
         * Gets the output image component (for external access)
         */
        getImageComponent() {
        ...
        /**
         * Checks if this node has a generated image ready for output
         */
        hasGeneratedImage() {
            return this._generatedTexture !== null;
        }
        /**
         * Gets all data that can be passed to other nodes
         * Returns the generated texture and cloned material
         * Compatible with InputNodeImage interface for connection handling
         */
        getOutputData() {
            return {
