        ...
        createTextConnection(sourceNode, promptNode, connectionId) {
            ...
            this.createEvent("UpdateEvent").bind(() => {
                ...
                try {
                    ...
                    if (bezierCurve) {
                        // FIRST: Set temporary placeholders to satisfy validation
                        ...
                        // Wait another frame, then update to real objects
                        ...
                        this.createEvent("UpdateEvent").bind(() => {
                            if (updateExecuted)
                                return;
                            updateFrameCount++;
                            if (updateFrameCount < 1) {
                                return;
                            }
                            updateExecuted = true;
                            try {
                                // NOW update to real objects
                                ...
                                print(`ProcessImageGenNode: ERROR - Failed to update BezierCurve: ${updateError}`);
                                tempStartPoint.destroy();
                                tempEndPoint.destroy();
                            }
                        });
        ...
        displayGeneratedImage(texture) {
            ...
            if (this.outputImage && this.outputImage.mainPass) {
                ...
                if (!material) {
                    ...
                    // Clone the material to ensure independence from input nodes
                    // This prevents material sharing issues
                    try {
                        const clonedMaterial = material.clone();
                        this.outputImage.mainPass.material = clonedMaterial;
                        material = clonedMaterial;
                    }
                    catch (e) {
                        // If cloning fails, use existing material (shouldn't happen in normal flow)
                ...
                print("ProcessImageGenNode: Generated image displayed with independent material");
            }
            else {
