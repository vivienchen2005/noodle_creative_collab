...
export class InputNodePrompt extends BaseScriptComponent {
    ...
    private setupOutputButton(): void {
        ...
        if (this.outputButton) {
            // Get reference to the button's SceneObject and ensure it's enabled
            if (this.outputButton.sceneObject) {
                this._outputButtonObject = this.outputButton.sceneObject;
                this._outputButtonObject.enabled = true;
                print("InputNodePrompt: Found existing output button, ensuring it's enabled");
            }
        ...
        if (this.outputButton) {
            ...
            print("InputNodePrompt: Created output button (RoundButton)");
        }
    }
    ...
    private onOutputButtonClicked(): void {
        print(`InputNodePrompt: Output button clicked (ID: ${this._outputButtonId}) - Prompt text: "${this.getPromptText()}"`);

        // Notify connection controller to start pending connection
        const controller = NodeConnectionController.getInstance();
        if (controller) {
            controller.onInputNodeButtonClicked(this.sceneObject, "text", this._outputButtonId);
        } else {
            print(`InputNodePrompt: WARNING - NodeConnectionController not found! Attempting to find or create...`);
    ...
    private findOrCreateConnectionController(): void {
        ...
        if (controller) {
            print(`InputNodePrompt: Found NodeConnectionController (singleton)`);
            controller.onInputNodeButtonClicked(this.sceneObject, "text", this._outputButtonId);
            return;
        }
        ...
        for (let i = 0; i < rootObjects; i++) {
            ...
            if (controller) {
                print(`InputNodePrompt: Found NodeConnectionController in scene`);
                controller.onInputNodeButtonClicked(this.sceneObject, "text", this._outputButtonId);
                return;
            }
        }
        ...
        print(`InputNodePrompt: Creating NodeConnectionController in scene...`);
        ...
        if (controller) {
            print(`InputNodePrompt: NodeConnectionController created (material can be assigned later)`);
            // Wait a frame for component to initialize
            this.createEvent("UpdateEvent").bind(() => {
                controller.onInputNodeButtonClicked(this.sceneObject, "text", this._outputButtonId);
            });
        } else {
            print(`InputNodePrompt: ERROR - Failed to create NodeConnectionController component`);
        }
    }
     ...
     */
    public getPromptTextComponent(): Text | null {
